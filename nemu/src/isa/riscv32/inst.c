...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm;       R(rd) = s->snpc; ); //添加J指令
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 00100 11", sltu   , I, R(rd) = ((int32_t)src1 < (int32_t)src2) ? 1 : 0);
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slt    , I, R(rd) = (src1 < src2) ? 1 : 0);
